        ...
        <tbody>
          {payments?.purchasedItem?.map((purce,index) => (
            <tr key={index} className="text-center border-b hover:bg-gray-50">
              ...
              <td className="py-2 px-4">{ getTimeAndDate(purce.addedDate)}</td>
              <td className={`py-2 px-4 font-semibold ${purce.paid_status=='pending'? 'text-red-500':'text-green-500'}`}>{purce.paid_status}</td>
