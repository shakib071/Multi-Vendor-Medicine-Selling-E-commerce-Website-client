  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-[#ffffff] rounded-xl shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Paid Total Card */}
        <div className="flex items-center p-3 md:p-6 bg-green-200 rounded-lg shadow-md">
          <div className="p-4 bg-green-400 rounded-full mr-6 text-green-900">
            <FaDollarSign size={36} />
          </div>
          <div>
            <h3 className="text-[17px] md:text-lg font-semibold text-green-800 mb-1">Paid Total</h3>
            <p className="text-2xl md:text-3xl font-bold text-green-900">
              ${getPaidTotal()?.toFixed(2)}
            </p>
            <p className="text-green-700 text-xs md:text-sm mt-1">Revenue successfully collected</p>
          </div>
        </div>

        {/* Pending Total Card */}
        <div className="flex items-center p-3 md:p-6 bg-yellow-200 rounded-lg shadow-md">
          <div className="p-4 bg-yellow-50 rounded-full mr-6 text-yellow-700">
            <FaClock size={36} />
          </div>
          <div>
            <h3 className="text-[17px] md:text-lg font-semibold text-red-800 mb-1">Pending Total</h3>
            <p className="text-2xl md:text-3xl font-bold text-red-900">
              ${getPendingTotal()?.toFixed(2)}
            </p>
            <p className="text-yellow-700 text-xs md:text-sm mt-1">Revenue awaiting payment</p>
          </div>
        </div>
      </div>
    </div>
